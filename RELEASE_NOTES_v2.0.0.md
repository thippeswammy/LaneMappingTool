...
2. **Transfer Files**:
   ...
   - **Destination**: `AGC_ws/Network/` on the vehicle (Ubuntu/ROS1 system).
...
3. **Convert to Pickle (On Vehicle)**:
   ...
   - Run the conversion script using the vehicle's python (Python 2.7):
     ...
     ```
   - This generates the Python 2.6/2.7 compatible `.pickle` file required by the autonomous stack.

4. **Verify & Restart**:
   - Confirm `output.pickle` is created.
   - Restart the vehicle software stack.

## 📦 Upgrade Notes
- This release updates the internal data format handling. It is recommended to back up existing `output.pickle` files before editing.
