...
class DataManager:
    def __init__(self, data, file_names):
        if data.size > 0:
            if len(data.shape) != 2 or data.shape[1] < 4:
                raise ValueError(
                    f"Expected data to be a 2D array with at least 4 columns (x, y, yaw, ..., index, lane_id), got shape {data.shape}")
    ...
    def add_point(self, x, y, lane_id):
        ...
        new_point[0, -2] = len(self.data)
        self.data = np.vstack([self.data, new_point]) if self.data.size > 0 else new_point
        self.history.append(self.data.copy())
        self.redo_stack = []
        print(
            f"Added point: ({x}, {y}, index={new_point[0, -2]}, lane_id={lane_id}), new data shape: {self.data.shape}")
    ...
    def delete_points(self, indices):
        ...
        if self.data.size > 0:
            self.data[:, -2] = np.arange(len(self.data))
        self.history.append(self.data.copy())
        self.redo_stack = []
    ...
    def save(self):
        ...
        if self.data.size > 0:
            np.save(filename, self.data[:, :3])  # Save only x, y
        else:
            np.save(filename, np.array([]))
        print(f"Saved x, y to {filename}")
        return filename
